<?php
mail('company703@gmail.com','stimma from b24_2 index.php',  'stimma from b24_2 index_old.php');
...
if (isset($json)) {
    ...
    if ($arJSON['type'] == 'order') {
        if (!empty($arJSON['line_items'])) {
            ...
            $arProducts = array();
            $arOrderProductsAll = array();
            $arProductsListResultAll = array();
            ...
            for ($i = 0; $i < $steps; $i++) {
                ...
                $arFieldsProductList = array(
                    "order" => array("NAME" => "ASC"),
                    "filter" => array("PROPERTY_104" => $arOrderProductsStep),
                    "select" => array("ID", "NAME", "CURRENCY_ID", "PRICE", "PROPERTY_*")
                );
                while (1) {
                    $arProductListResult = call("crm.product.list", $arFieldsProductList, true);
                    if ($arProductListResult['STATUS'] != 503) break;
                }
            ...
            if ($arJSON['customer_id'] > 0) {
                $arFilter = array("UF_CRM_1521453922" => $arJSON['customer_id']);
            } else {
                $arFilter = (string)$arJSON['billing']->email ? array("EMAIL" => (string)$arJSON['billing']->email) : array("PHONE" => (string)$arJSON['billing']->phone);
            }
            $arContactsParams = array(
                "order" => array("ID" => "ASC"),
                "filter" => $arFilter,
                "select" => array("ID", "EMAIL", "UF_CRM_1521453922", "ASSIGNED_BY_ID", "PHONE", "TYPE_ID")
            );
            while (1) {
                $arContactsResult = call("crm.contact.list", $arContactsParams, true);
                if ($arContactsResult['STATUS'] == 200) break;
            }
            // поиск контакта end
            ...
            if (count($arContactsResult['result']) > 0) {
                // поиск сделки start
                $arFieldsDealsList = array(
                    "order" => array("ID" => "ASC"),
                    "filter" => array("UF_CRM_1523544904" => $arJSON['id']),
                    "select" => array("ID", "NAME", "UF_CRM_1523544904")
                );
                while (1) {
                    $arDealListResult = call("crm.deal.list", $arFieldsDealsList, true);
                    if ($arDealListResult['STATUS'] == 200) break;
                }
                ...
                if (empty($arDealListResult['result'])) {
                    ...
                    $arDealParams = array(
                        "fields" => array(
                            "CATEGORY_ID" =>  $arContactsResult['result'][0]["TYPE_ID"] ? $arClientDealCategory[$arContactsResult['result'][0]["TYPE_ID"]] : 0,
                            "TITLE" => "Замовлення № " . $arJSON['id'],
                            "TYPE_ID" => "SALE",
                            // "STAGE_ID" => "NEW",
                            "STAGE_ID" => $arContactsResult['result'][0]["TYPE_ID"] ? $arClientDealSatus[$arContactsResult['result'][0]["TYPE_ID"]] : "NEW",
                            "CONTACT_ID" => $arContactsResult['result'][0]['ID'],
                            "OPENED" => "Y",
                            "ASSIGNED_BY_ID" => $arContactsResult['result'][0]['ASSIGNED_BY_ID'],
                ...
                $phone = array(array("VALUE" => $phone_tmp, "VALUE_TYPE" => "WORK"));
                $arFieldsContactAdd = array(
                    "fields" => array(
                        "OPENED" => 'Y',
                        "EXPORT" => 'Y',
                        "TYPE_ID" => 'CLIENT', // категория клиента (оптовик, розничный покупатель)
                        "SOURCE_ID" => "WEB",
                        "NAME" => $arJSON['billing']->first_name, // Имя
                        "LAST_NAME" => $arJSON['billing']->last_name, // Фамилия
                        "PHONE" => $phone, // Телефон
                        "EMAIL" => array(array("VALUE" => $arJSON['billing']->email, "VALUE_TYPE" => "WORK")), // Email
                ...
                if ($contact_result['result'] && $contact_result['result'] > 0) {
                    ...
                    if (empty($arDealListResult['result'])) {
                        ...
                        $arDealParams = array(
                            "fields" => array(
                                "CATEGORY_ID" =>  $arClientDealCategory["CLIENT"],
                                "TITLE" => "Замовлення № " . $arJSON['id'],
                                "TYPE_ID" => "SALE",
                                "STAGE_ID" => "NEW",
        ...
        $arFieldsProductList = array(
            "order" => array('ID' => 'ASC'),
            "filter" => array('PROPERTY_104' => $arJSON['id']),
            "select" => array('ID', 'NAME', 'PRICE', 'PROPERTY_*')
        );
        while (1) {
            $arProductListResult = call("crm.product.list", $arFieldsProductList, true);
            if ($arProductListResult['STATUS'] == 200) break;
        }
        ...
        if (empty($arProductListResult['result'])) {
            ...
            $arFieldsProductAdd = array(
                "fields" => array(
                    ...
                    "CURRENCY_ID" => "UAH",
                    "PRICE" => $arJSON['price'],
                    "PROPERTY_100" => (string)$arJSON['attributes']->pa_cvet, // цвет
            ...
            $arFieldsProductUpdate = array(
                "id" => $arProductListResult['result'][0]['ID'],
                "fields" => array(
                    "NAME" => (string)$arJSON['name'],
                    "CURRENCY_ID" => "UAH",
                    "PRICE" => (string)$arJSON['price'],
                    "PROPERTY_100" => (string)$arJSON['attributes']->pa_cvet, // цвет
                    "PROPERTY_102" => (string)$arJSON['attributes']->pa_razmer, // размер
                    "PROPERTY_104" => $arJSON['id'], // id на сайте
                    "PROPERTY_106" => (string)$arJSON['attributes']->pa_material, // материал
            ...
            while (1) {
                ...
                if ($arProductUpdateResult['STATUS'] == 200) break;
            }
        ...
        if ($arJSON['user_type'] == 'opt') {
            ...
            $role = '1';
            $name_role = "Опт";
            $arFieldsContactsListNew = array(
                "order" => array("DATE_CREATE" => "DESC"),
                "filter" => array("ASSIGNED_BY_ID" => $arOptManagers),
                "select" => array("ID", "ASSIGNED_BY_ID", "DATE_CREATE")
            );
            while (1) {
                $arContactListResultNew = call("crm.contact.list", $arFieldsContactsListNew, true);
                if ($arContactListResultNew['STATUS'] == 200) break;
            }
        ...
        $arFieldsContactAdd = array(
            "fields" => array(
                "OPENED" => 'Y',
                "EXPORT" => 'Y',
                "TYPE_ID" => $role, // категория клиента (оптовик, розничный покупатель)
                "SOURCE_ID" => "WEB",
                "NAME" => $arJSON['user_firstname'], // Имя
                "LAST_NAME" => $arJSON['user_lastname'], // Фамилия
                "PHONE" => $phone, // Телефон
                "EMAIL" => array(array("VALUE" => $arJSON['user_email'], "VALUE_TYPE" => "WORK")), // Email
        ...
        $arLeadParams = array(
            "fields" => array(
                "TITLE" => $title_lid, 
                "NAME" => $arJSON['user_firstname'], 
                "LAST_NAME" => $arJSON['user_lastname'], 
                "STATUS_ID" => "NEW",
                "SOURCE_ID" => "WEB",
                "OPENED" => "Y", 
               // "ASSIGNED_BY_ID" =>  $role, 
