...
<?
{
    ...
    while ($record = $res -> GetNextElement())
    {
        ...
        if($props['NAME_UA']['VALUE']) $fields['NAME'] = $props['NAME_UA']['VALUE'];
        if($props['ADDRESS_UA']['VALUE']) $props['ADDRESS']['VALUE'] = $props['ADDRESS_UA']['VALUE'];
        if($props['SCHEDULE_UA']['VALUE']) $props['SCHEDULE']['VALUE'] = $props['SCHEDULE_UA']['VALUE'];
        if($props['METRO_UA']['VALUE']) $props['METRO']['VALUE'] = $props['METRO_UA']['VALUE'];
    ...
    if(!empty($sections))
    {
        $res = CIBlockSection::GetList([], ['ID' => $sections,'IBLOCK_ID' => 12], false, ['ID','IBLOCK_ID','NAME','UF_*']);
        while ($record = $res -> Fetch())
        {
            if($record['UF_NAME_UA']) $record['NAME'] = $record['UF_NAME_UA'];
