	...
	<div class="maxwidth-theme only-on-front">
		...
		<div class="item-views reviews<?=($bCompact ? ' compact' : '');?><?=($bMoreItem ? ' more-item' : '');?> <?=($arParams['LINKED_MODE'] == 'Y' ? ' linked ' : '');?> <?=($notSlider ? ' list-mode ' : '');?>">
			...
			<?endif;?>
                ...
                if(LANGUAGE_ID=='ua')
                {
                    $arItem['DISPLAY_ACTIVE_FROM'] = str_replace(['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря',],
                                                                 ['січня','лютого','березня','квітня','травня','червня','липня','серпня','вересня','жовтня','листопада','грудня',],$arItem['DISPLAY_ACTIVE_FROM']);
                }
					...
					<div class="item-wrapper col-xs-12<?=(!$bOneItem ?  ' bg-fill-white bordered1 box-shadow1' : '');?>">
						<div  class="item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<div class="top_wrapper clearfix">
								...
								<?endif;*/?>
								<div class="top-info">
									<div class="wrap muted" style="font-weight: 700;">
										<?if(isset($arItem['NAME']) && strlen($arItem['NAME'])):?>
											<span class="font_upper"><?=$arItem['NAME']?></span>
										<?endif?>
										<?if(isset($arItem['NAME']) && $arItem['NAME'] && isset($arItem['PREVIEW_TEXT']) && strlen($arItem['PREVIEW_TEXT'])):?>
									...
									<div class="title <?=($bCompact ? 'font_md' : 'font_lg');?> pull-left" style="color:#3D441D;font-weight: normal;font-size: 1.1333em;">
										<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
										<?=$arItem['PREVIEW_TEXT'];?>
										<?if($bDetailLink):?></a><?endif;?>
									</div>
						...
						<?	$answer = (isset($arItem['FIELDS']['DETAIL_TEXT']) && strlen($arItem['FIELDS']['DETAIL_TEXT']) ? $arItem['FIELDS']['DETAIL_TEXT'] : '');
							...
							?>
							<div class="respone_wrap rounded2 bordered  <?=($bImageResp ? 'with-img ' : '')?>" style="color:black;">
								...
								<div class="response_body">
									...
									<?endif;?>
									<div class="response_text muted777 font_xs" style="color:black;"><?=$answer;?></div>
								</div>
							</div>
						<?endif;?>
					</div>
