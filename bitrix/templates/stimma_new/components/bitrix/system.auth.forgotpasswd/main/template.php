		...
		<div class="form">
			<form id="forgotpasswd-page-form" method="post" action="<?=POST_FORM_ACTION_URI?>" name="bform">
				...
				<div class="form_body">
					...
					<?else:?>
						<div class="form-control set_email">
							<label for="FORGOTPASSWD_USER_LOGIN"><span><?=GetMessage('AUTH_LOGIN')?>&nbsp;<span class="star">*</span></span></label>
							<input id="FORGOTPASSWD_USER_LOGIN" type="<?=($bEmailAsLogin ? 'email' : 'text')?>" name="USER_LOGIN" required maxlength="255" autocomplete="off" value="<?=$_GET['USER_EMAIL']?>"/>
							<input type="hidden" name="USER_EMAIL" maxlength="255" autocomplete="off" value=""/>
							<div class="text-block"><?=GetMessage('forgot_pass_login_note')?></div>
						</div>
						<div class="form-control set_code" style="<?=isset($_GET['USER_EMAIL']) ? '' : 'display:none;'?>">
							<label for=""><span><?=GetMessage('SET_CODE')?>&nbsp;<span class="star">*</span></span></label>
							<input id="" type="text" name="USER_CODE" required maxlength="255" autocomplete="off" value="<?=$_GET['USER_CHECKWORD']?>"/>
						</div>
						<div class="form-control set_pass" style="<?=isset($_GET['USER_EMAIL']) ? '' : 'display:none;'?>">
							<label for=""><span><?=GetMessage('SET_PASS')?>&nbsp;<span class="star">*</span></span></label>
							<input id="" type="text" name="USER_PASS" required maxlength="255" autocomplete="off" value=""/>
						</div>
						<div class="form-control set_repeat_pass" style="<?=isset($_GET['USER_EMAIL']) ? '' : 'display:none;'?>">
							<label for=""><span><?=GetMessage('SET_REPEAT_PASS')?>&nbsp;<span class="star">*</span></span></label>
							<input id="" type="text" name="USER_REPEAT_PASS" required maxlength="255" autocomplete="off" />
						</div>
					<?endif;?>
