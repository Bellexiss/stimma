...
$colorVariants = $noId = $Ids = $json = $jsonVariants = [];
$type = getTypeDevice();
...
if(LANGUAGE_ID == 'ua')
{
    ...
    foreach ($arResult['ITEMS'] as $index => $arItem)
    {
        if(isset($_GET['show'])) echo $arItem['ID'].'<br>';
        $itemSizes=[];
        ...
        foreach ($arItem['OFFERS'] as $indexOffer => $offer)
        {
            ...
            $size=str_replace('_','-',$offer['PROPERTIES']['RAZMER']['VALUE']);
            $itemSizes[$size] = $offer['ID'];
            //$nameUrl = CUtil::translit($arResult['ITEMS'][$index]['NAME'],'ru');
            //$jsonVariants[$nameUrl]['SIZES'][$size] = $offer['ID'];
        }
        ...
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['id'] = $arItem['ID'];
        ...
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['SIZES'] = $itemSizes;
        $nameUrl = CUtil::translit($arResult['ITEMS'][$index]['NAME'],'ru');
        $jsonVariants[$nameUrl]['COLORS'] = $colorVariants[$arItem['NAME']];
        unset($itemSizes);
        //$jsonVariants[$nameUrl]['COLORS'][$arItem['PROPERTIES']['COLOR']['VALUE']]['SIZES'] = $itemSizes;
    }
}
else
{
    foreach ($arResult['ITEMS'] as $index => $arItem)
    {
        ...
        foreach ($arItem['OFFERS'] as $indexOffer => $offer)
        {
            $size=str_replace('_','-',$offer['PROPERTIES']['RAZMER']['VALUE']);
            $itemSizes[$size] = $offer['ID'];
            //$nameUrl = CUtil::translit($arItem['NAME'],'ru');
            //$jsonVariants[$nameUrl]['SIZES'][$size] = $offer['ID'];
        }
        //OFFER_ID_SELECTED
        $item = $arItem['OFFERS'][0];
        ...
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['id'] = $item['ID'];
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['img'] = $img;
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['price'] = $minPrice;
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['code'] = $arItem['CODE'];
        $colorVariants[$arItem['NAME']][$arItem['PROPERTIES']['COLOR']['VALUE']]['name'] = $arItem['NAME'];
...
if (!empty($colorVariants))
{
    ...
    while ($record = $res -> Fetch())
    {
        ...
        $colorVariants[$record['NAME']][$record['PROPERTY_COLOR_VALUE']]['id'] = $record['ID'];
        ...
        $nameUrl = CUtil::translit($record['NAME'],'ru');
        $jsonVariants[$nameUrl]['COLORS'] = $colorVariants[$record['NAME']];
    }
}
...
if(isset($_GET['show']))
{
    ...
    ?><pre><?=print_r($jsonVariants,1)?></pre><?
    die();
}
$arResult['JSON_VARIANTS'] = $jsonVariants;
