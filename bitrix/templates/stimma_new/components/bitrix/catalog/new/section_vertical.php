...
if(LANGUAGE_ID == 'ua')
{
    ...
    if($section['UF_NAME_UA'])
        ...
        $section = $originalSection['NAME'] = $section['NAME'];
}
else
{
...
if(isset($_GET['kk']))
{
    ?><pre><?=print_r($originalSection, 1)?></pre><?
}
...
if($isNewPage)
{
    if(LANGUAGE_ID == 'ru')
    {
        $originalSeo = $APPLICATION->IncludeComponent(
            "dwstroy:dwstroy.seo.section",
            "",
            Array(
                "SECTION_ID" => $originalSection['ID'], //Ид раздела
    ...
    {
        $originalSeo = $ipropSectionValues = new \Bitrix\Iblock\InheritedProperty\SectionValues($arParams['IBLOCK_ID'], $originalSection['ID']);
        $originalSeo = $originalSeo->getValues();
...
<div class="wrapper">
    <div class="catalog-page">
                ...
                */?>
                ...
                if($isNewPage)
                {
                    ...
                    <h1 class="catalog-page-title">
                        <?//$APPLICATION->ShowViewContent('mdf_title2');?>
        ...
        <?//if(isset($_GET['semicat']))
        {?>
            <?
            $subSections = [];
            $res = CIBlockSection::GetList(['sort'=>'asc'], ['IBLOCK_ID' => $arParams['IBLOCK_ID'],'ACTIVE'=>'Y','SECTION_ID'=>$originalSection['ID']],false,['UF_*']);
            while ($subSection = $res->GetNext())
            {
                if(LANGUAGE_ID == 'ua')
                    $subSection['NAME'] = $subSection['UF_NAME_UA'] ? $subSection['UF_NAME_UA'] : $subSection['NAME'];

                $subSections[] = $subSection;
            }
            ?>
        <div class="catalog-title-cont">
            <h1 class="catalog-title">
                <?
                //if(!$seo['ELEMENT_PAGE_TITLE'])
                    //$seo['ELEMENT_PAGE_TITLE'] = $originalSection['NAME'];
                /*if(isset($_GET['pp']))
                {
                    ?><pre>$selectedFilter <?=print_r($selectedFilter, 1)?></pre><?
                }*/
                if($isNewPage || $isH1)
                    echo $originalSeo['SECTION_PAGE_TITLE'] ? $originalSeo['SECTION_PAGE_TITLE'] : $originalSection['NAME'];
                else
                    $APPLICATION->ShowViewContent('mdf_title');?>
            </h1>
            <?
            $ru = LANGUAGE_ID == 'ru' ? '/ru' : '';
            if(in_array($originalSection['ID'],[360,405,406]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/kostyumy_i_zhakety/" class="<?=$originalSection['ID'] == 360 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/kostyumy_i_zhakety/zhakety/" class="<?=$originalSection['ID'] == 406 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Жакети' : 'Жакеты'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/kostyumy_i_zhakety/zhenskie_kostyumy/" class="<?=$originalSection['ID'] == 405 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Костюми' : 'Костюмы'?>
                        </a>
                    </div>
                </div>
                <?
            }
            if(in_array($originalSection['ID'],[1339,1182,1340]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/zhilety_i_korsety/" class="<?=$originalSection['ID'] == 1339 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/kostyumy_i_zhakety/zhilet/" class="<?=$originalSection['ID'] == 1182 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Жилети' : 'Жилеты'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/zhilety_i_korsety/korsety/" class="<?=$originalSection['ID'] == 1340 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Корсети' : 'Корсеты'?>
                        </a>
                    </div>
                </div>
                <?
            }
            if(in_array($originalSection['ID'],[359,403,404]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/bluzy_i_rubashki/" class="<?=$originalSection['ID'] == 359 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/bluzy_i_rubashki/rubashki/" class="<?=$originalSection['ID'] == 403 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Сорочки' : 'Рубашки'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/bluzy_i_rubashki/bluzy/" class="<?=$originalSection['ID'] == 404 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Блузи' : 'Блузы'?>
                        </a>
            ...
            if(in_array($originalSection['ID'],[354,375,377,376,393]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/platya_sarafany_i_yubki/" class="<?=$originalSection['ID'] == 354 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/platya_sarafany_i_yubki/platya/" class="<?=$originalSection['ID'] == 375 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Сукні' : 'Платья'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/platya_sarafany_i_yubki/sarafany/" class="<?=$originalSection['ID'] == 377 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Сарафани' : 'Сарафаны'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/platya_sarafany_i_yubki/yubki/" class="<?=$originalSection['ID'] == 376 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Спідниці' : 'Юбки'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/platya_trikotazh/" class="<?=$originalSection['ID'] == 393 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Сукні трикотажні' : 'Платья трикотажные'?>
                        </a>
            ...
            if(in_array($originalSection['ID'],[355,379,381,382]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/dzhinsy_bryuki_shorty/" class="<?=$originalSection['ID'] == 355 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/dzhinsy_bryuki_shorty/bryuki/" class="<?=$originalSection['ID'] == 379 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Штани' : 'Штаны'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/dzhinsy_bryuki_shorty/shorty/" class="<?=$originalSection['ID'] == 381 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Шорти' : 'Шорты'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/dzhinsy_bryuki_shorty/legginsy/" class="<?=$originalSection['ID'] == 382 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Легінси' : 'Легинсы'?>
                        </a>
            ...
            if(in_array($originalSection['ID'],[1254,378,1257]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/denim/" class="<?=$originalSection['ID'] == 1254 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/dzhinsy_bryuki_shorty/dzhinsy/" class="<?=$originalSection['ID'] == 378 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Джинси' : 'Джинсы'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/denim/dzhinsovye_shorty/" class="<?=$originalSection['ID'] == 1257 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Джинсові шорти' : 'Джинсовые шорты'?>
                        </a>
            ...
            if(in_array($originalSection['ID'],[358,399,400,401]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/topy1/" class="<?=$originalSection['ID'] == 358 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/topy1/futbolki/" class="<?=$originalSection['ID'] == 399 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Футболки' : 'Футболки'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/topy1/topy/" class="<?=$originalSection['ID'] == 400 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Топи' : 'Топы'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/topy1/longslivy/" class="<?=$originalSection['ID'] == 401 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Лонгсліви' : 'Лонгсливы'?>
                        </a>
            ...
            if(in_array($originalSection['ID'],[357,394,395,398,507]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/sportivnaya_odezhda/" class="<?=$originalSection['ID'] == 357 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/sportivnaya_odezhda/sportivnye_kostyumy/" class="<?=$originalSection['ID'] == 394 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Спортивні костюми' : 'Спортивные костюмы'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/sportivnaya_odezhda/sportivnye_shtany1/" class="<?=$originalSection['ID'] == 395 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Штани' : 'Штаны'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/sportivnaya_odezhda/shorty_g/" class="<?=$originalSection['ID'] == 398 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Спортивні шорти' : 'Спортивные шорты'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/sportivnaya_odezhda/khudi_i_svitshoty/" class="<?=$originalSection['ID'] == 507 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Худі та світшоти' : 'Худи и свитшоты'?>
                        </a>
            ...
            if(in_array($originalSection['ID'],[356,389,392,383,1275]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/" class="<?=$originalSection['ID'] == 356 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/kardigany/" class="<?=$originalSection['ID'] == 389 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Кардигани' : 'Кардиганы'?>
                        </a>
                        <?/*<a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/platya_trikotazh/" class="<?=$originalSection['ID'] == 393 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Сукні та спідниці' : 'Платья и юбки'?>
                        </a>*/?>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/zhilety_trikotazh/" class="<?=$originalSection['ID'] == 392 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Жилетки' : 'Жилетки'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/dzhempery/" class="<?=$originalSection['ID'] == 383 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Светри та джемпери' : 'Светры и джемперы'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/trikotazh/kostyumy_i_komplekty/" class="<?=$originalSection['ID'] == 1275 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Костюми і комплекти' : 'Костюмы и комплекты'?>
                        </a>
                    </div>
                </div>
                <?
            }
            if(in_array($originalSection['ID'],[353,368,369,370,371,1334,1322]))
            {
                ?>
                <div class="catalog-semicategory-list-cont">
                    <div class="catalog-semicategory-list">
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/" class="<?=$originalSection['ID'] == 353 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Переглянути все' : 'Посмотреть все'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/zimnie_kurtki_/" class="<?=$originalSection['ID'] == 368 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Зимові куртки' : 'Кардиганы'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/demisezonnye_kurtki/" class="<?=$originalSection['ID'] == 369 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Демісезонні куртки' : 'Платья и юбки'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/bombery/" class="<?=$originalSection['ID'] == 370 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Бомбери' : 'Жилетки'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/palto/" class="<?=$originalSection['ID'] == 371 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Пальто' : 'Светры и джемперы'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/trenchi/" class="<?=$originalSection['ID'] == 1334 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Тренчі' : 'Костюмы и комплекты'?>
                        </a>
                        <a href="<?=$ru?>/catalog/zhenskaya_odezhda/verkhnyaya_odezhda/shuby/" class="<?=$originalSection['ID'] == 1322 ? 'active' : ''?>">
                            <?=LANGUAGE_ID == 'ua' ? 'Шуби' : 'Костюмы и комплекты'?>
                        </a>
                    </div>
                </div>
                <?
            ...
            <div class="catalog-faq-list">
                <div class="accordion">
                    ...
                    foreach ($questions as $index => $question)
                    {
                        ...
                        <div class="accordion-item">
                            <div class="accordion-header">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#panels<?=($index+1)?>" aria-expanded="false" >
                                    <?=str_replace('\'','’', $question['UF_QUESTION'])?>
                                </button>
                            </div>
                            <div id="panels<?=($index+1)?>" class="accordion-collapse collapse">
                                <div class="accordion-body">
                                    <?=str_replace('\'','’', $question['UF_ANSWER'])?>
                                </div>
                            </div>
                        </div>
                        <?
                        $json[]='{
                    "@type": "Question",
                    "name": "'.addslashes(str_replace("'",'’',$question['UF_QUESTION'])).'",
                    "acceptedAnswer": {
                        "@type": "Answer",
                        "text": "'.addslashes(str_replace("'",'’',$question['UF_ANSWER'])).'"
                    }
                }';
                    }
                    ?>
...
if(!isset($_GET['PAGEN_1']) && !$bIndex)
{
    if(isset($_SERVER['ELEMENT_ID']) && $_SERVER['ELEMENT_ID'] > 0)
    {
        ...
        if($seoProps['SEO_RU_TEXT']['~VALUE']['TEXT'] && LANGUAGE_ID=='ru')
        {
            ...
            <div class="col-md-12">
                <div class="seo_block">
                    <div class="seo_text col-md-12" >
                        <?=$seoProps['SEO_RU_TEXT']['~VALUE']['TEXT']?>
                    </div>
                    <div class="seo_block_toggler_block">
                        <div class="seo_block_toggler">
                            <span class="seo_open">
                                Показати більше
                            </span>
                            <span class="seo_close">
                                Сховати
                            </span>
                        </div>
                    </div>
    ...
    {
        if((LANGUAGE_ID=='ua' && $section['UF_DETAIL_TEXT_UA']) || (LANGUAGE_ID!='ua' && $section['DESCRIPTION']))
        {
            ?>
            <div class="col-md-12">
                <div class="seo_block">
