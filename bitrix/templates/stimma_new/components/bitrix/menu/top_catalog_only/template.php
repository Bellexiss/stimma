			...
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					<td class="menu-item <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($bWideMenu ? 'wide_menu' : '');?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							<a class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" href="<?=$arItem['PARAMS']['URL'] ? $arItem['PARAMS']['URL'] : $arItem["LINK"]?>">
								<div>
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<div class="dropdown-menu wrb3 <?=$bRightSideShow ? 'with_right_block' : ''?> <?=$bManyItemsMenu ? 'long-menu-items' : ''?> <?=$arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']?>" >
									<?if($bManyItemsMenu):?>
												...
												<?if($MENU_TYPE == 2 && $bWideMenu):?>
													...
													<li class="<?=($arSubItem["SELECTED"] ? "active" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($bHasPicture ? "has_img" : "")?>">
														<?if($bHasPicture && $bWideMenu):
															...
															<?endif;?>
														<?endif;?>
														<a href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name option-font-bold nofb5" style="<?=mb_strtolower($arSubItem["TEXT"]) == 'sale' ? 'color:#8B0000;' : ''?>"><?=$arSubItem["TEXT"]?></span></a>
													</li>
