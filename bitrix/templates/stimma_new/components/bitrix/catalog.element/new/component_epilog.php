...
# Доповнити свій образ
global $dopObraz;
$dopObraz=[];

$ids2=[];
$cml2link = $DB->Query('select * from b_iblock_element_property where IBLOCK_PROPERTY_ID = 390 and IBLOCK_ELEMENT_ID = '.$arResult['ID']);
...
//if(!empty($ids2))
{
    ...
    {
        $res2=$DB->Query('select * from b_catalog_product where ID = '.$record['VALUE'])->Fetch();
        if($res2['QUANTITY']>0);
