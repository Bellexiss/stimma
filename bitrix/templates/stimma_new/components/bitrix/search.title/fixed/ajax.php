...
$search = $_REQUEST['q'];
$ids = [];
$res = $DB -> Query('select * from b_iblock_element where LOWER(NAME) like \'%'.strtolower($search).'%\' and IBLOCK_ID = 21 and ACTIVE = \'Y\' limit 10');
...
if(!empty($ids))
{
    $res = CIBlockElement::GetList([], ['IBLOCK_ID' => 21, 'ID' => $ids]);
