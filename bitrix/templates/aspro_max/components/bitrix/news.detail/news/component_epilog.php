							...
							if($arSectionsID):?>
									...
									<?$APPLICATION->IncludeComponent(
										...
										Array(
											...
											"SECTION_USER_FIELDS" => array("UF_CATALOG_ICON",'UF_NAME_UA','UF_DETAIL_TEXT_UA'),
...
if(LANGUAGE_ID == 'ua')
{
    $res = CIBlockSection::GetNavChain($arParams['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID'], array('ID', 'NAME','UF_*'));
    ...
    $APPLICATION -> AddChainItem(trim($arResult['NAME']), '');

    $APPLICATION -> SetTitle(trim($arResult['NAME']));

    $seo['ELEMENT_META_DESCRIPTION'] = 'Блог STIMMA ⏩ '. trim($arResult['NAME']) .'. Читайте більше цікавого в нашому блозі.';
}
else
{
    $APPLICATION -> SetTitle(trim($arResult['NAME']) .' | Блог STIMMA');
    $seo['ELEMENT_META_DESCRIPTION'] = 'Блог STIMMA ⏩ '. trim($arResult['NAME']) .'. Читайте больше интересного в нашем блоге.';
}

$seo['ELEMENT_META_TITLE'] = trim($arResult['NAME']) .' | Блог STIMMA';
