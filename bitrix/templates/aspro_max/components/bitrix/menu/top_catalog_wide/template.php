					...
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($bWideMenu ? 'wide_menu' : '');?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<div class="dropdown-menu wrb4 <?=$bRightSide ? 'with_right_block '.$RightContent : ''?>" >
												...
												<?if($MENU_TYPE == 2):?>
													...
													<li class="<?=($arSubItem["SELECTED"] ? "active" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($bHasPicture ? "has_img" : "")?>">
														<?if($bHasPicture && $bWideMenu):
															...
															<?endif;?>
														<?endif;?>
														<a href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name option-font-bold nofb1" style="<?=mb_strtolower($arSubItem["TEXT"]) == 'sale' ? 'color:#8B0000;' : ''?>"><?=$arSubItem["TEXT"]?></span></a>
													</li>
												...
												<?else: // type 1?>
													...
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?>">
														<?if($bHasPicture && $bWideMenu):
															...
															<?endif;?>
														<?endif;?>
														<a href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name" style="<?=mb_strtolower($arSubItem["TEXT"]) == 'sale' ? 'color:#8B0000;' : ''?>"><?=$arSubItem["TEXT"]?></span><?=($arSubItem["CHILD"] && $bShowChilds ? '<span class="arrow"><i></i></span>' : '')?></a>
