...
<?if($arResult['ITEMS']):?>
	...
	<div class="maxwidth-theme only-on-front">
		...
		?>
                            <h2 class="last_review_title"><?=GetMessage('M_LAST_REVIEW');?></h2>
		<div class="item-views reviews<?=($bCompact ? ' compact' : '');?><?=($bMoreItem ? ' more-item' : '');?> <?=($arParams['LINKED_MODE'] == 'Y' ? ' linked ' : '');?> <?=($notSlider ? ' list-mode ' : '');?>">
			...
			<?else:?>
				<div class="owl-carousel owl-theme owl-bg-nav short-nav hidden-dots visible-nav swipeignore wsmooth1 appear-block loading_state<?=(!$bOneItem ? ' shadow' : '');?>" data-plugin-options='{"nav": true, "dots": false, "loop": false, "marginMove": true, "autoplay": false, <?=($animationSpeed >= 0 ? '"smartSpeed": '.$animationSpeed.',' : '')?> "useCSS": true, "responsive": {"0":{"items": 1, "autoWidth": false, "lightDrag": true, "margin":-1}, "601":{"items": 1, "autoWidth": false, "lightDrag": false, "margin":0}, "1200":{"items": 1, "margin":-1}}}'>
			<?endif;?>
								...
								<div class="top-info">
									<div class="wrap muted">
										<?if(isset($arItem['NAME']) && strlen($arItem['NAME'])):?>
											<span class="font_upper" style="font-weight: 700;"><?=$arItem['NAME']?></span>
										<?endif?>
										...
										<?if(isset($arItem['DISPLAY_ACTIVE_FROM']) && $arItem['DISPLAY_ACTIVE_FROM']):?>
											<span class="date font_upper" style="font-weight: 700;"><?=$arItem['DISPLAY_ACTIVE_FROM']?></span>
										<?endif;?>
									</div>
									<div style="font-weight: normal;font-size: 16px;" class="title <?=($bCompact ? 'font_md' : 'font_lg');?> pull-left">
										<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
										<?=$arItem['PREVIEW_TEXT'];?>
										<?if($bDetailLink):?></a><?endif;?>
									</div>
