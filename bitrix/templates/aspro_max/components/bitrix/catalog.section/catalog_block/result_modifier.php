...
if(LANGUAGE_ID == 'ua')
    foreach ($arResult['ITEMS'] as $index => $arItem)
    {
        foreach ($arItem['JS_OFFERS'] as $indexOffer => $offer)
        {
            foreach ($offer['DISPLAY_PROPERTIES'] as $index => $arItem)
            {
                if(isset($arItem['VALUE_XML_ID'])|| $arItem['CODE'] == 'COLOR_REF')
                {
                    ...
                    $arResult['ITEMS'][$index]['JS_OFFERS'][$indexOffer]['DISPLAY_PROPERTIES'][$index]['VALUE'] = $result['values'];
                    $arResult['ITEMS'][$index]['JS_OFFERS'][$indexOffer]['DISPLAY_PROPERTIES'][$index]['NAME'] = $result['name'];
                }
            }
        }
    }
