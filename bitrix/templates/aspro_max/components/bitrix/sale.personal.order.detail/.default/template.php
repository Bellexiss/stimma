								...
								<div class="sale-order-detail-order-table-fade sale-order-detail-order-table-fade-right">
									<div style="width: 100%; overflow-x: auto; overflow-y: hidden;">
										<div class="sale-order-detail-order-item-table">
											...
											{
                                                $uaItem = CIBlockElement::GetByID($basketItem['PRODUCT_ID']) -> GetNextElement();
                                                $Props = $uaItem -> GetProperties();
                                                $Fields = $uaItem -> GetFields();

                                                $photo = $Fields['PREVIEW_PICTURE'];
                                                if(!$photo)
                                                    $photo = $Fields['DETAIL_PICTURE'];

                                                if(!$photo && $Props['CML2_LINK']['VALUE'])
                                                {
                                                    $uaItemMain = CIBlockElement::GetByID($Props['CML2_LINK']['VALUE']) -> Fetch();
                                                    $photo = $uaItemMain['PREVIEW_PICTURE'];
                                                    if(!$photo)
                                                        $photo = $uaItemMain['DETAIL_PICTURE'];
                                                }
                                                if(LANGUAGE_ID == 'ua')
                                                {
                                                    $basketItem['NAME'] = $Props['NAME_UA']['VALUE'];
                                                }
												?>
												<div class="sale-order-detail-order-item-tr sale-order-detail-order-basket-info sale-order-detail-order-item-tr-first">
															...
															<div class="sale-order-detail-order-item-img-block">
																<a href="<?=$basketItem['DETAIL_PAGE_URL']?>">
																	...
																	if($photo)
																	{
																		...
																		$imageSrc = CFile::GetFileArray($photo)['SRC'];
																	}
																	else
																	{
																		$imageSrc = $this->GetFolder().'/images/no_photo.png';
