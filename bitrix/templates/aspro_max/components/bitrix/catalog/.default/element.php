...
?>
		...
		if ($elementId > 0)
		{
            ...
            $res = $DB->Query('select * from b_iblock_section_element where IBLOCK_SECTION_ID = 350');
            $newIDs = [];
            while ($record = $res->Fetch())
            {
                $newIDs[$record['IBLOCK_ELEMENT_ID']] = $record['IBLOCK_ELEMENT_ID'];
            }
            $sid = CIBlockElement::GetByID($elementId) -> Fetch()['IBLOCK_SECTION_ID'];
            global $arViewed;
            //$arViewed = ['SECTION_ID' => 350,'>catalog_QUANTITY'=>0];
            $arViewed = ['ID' => $newIDs];
            $params['FILTER_NAME'] = 'arViewed';
            ?>
            <div class="main-googs-wraper mgw2">
                <div class="main-googs-wraper-title"><?=LANGUAGE_ID == 'ua' ? 'РЕКОМЕНДУЄМО' : 'Рекомендуем'?></div>
                <?
        ...
        ?>
            ...
            if(isset($_SESSION['VIEWED']) && !empty($_SESSION['VIEWED']))
            {
                ...
                $params['FILTER_NAME'] = 'arViewed';
                ?>
                <div class="main-googs-wraper mgw3">
