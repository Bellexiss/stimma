...
$section = CMaxCache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), CMax::makeSectionFilterInRegion($arSectionFilter), false, array('UF_NAME_UA','UF_DETAIL_TEXT_UA',"ID", "IBLOCK_ID", "NAME", "DESCRIPTION", "UF_SECTION_DESCR", "UF_OFFERS_TYPE", 'UF_FILTER_VIEW', 'UF_LINE_ELEMENT_CNT', 'UF_SECTION_BG_DARK', 'UF_LINKED_BLOG', 'UF_BLOG_BOTTOM', 'UF_BLOG_WIDE', 'UF_BLOG_MOBILE', 'UF_LINKED_BANNERS', 'UF_BANNERS_BOTTOM', 'UF_BANNERS_WIDE', 'UF_BANNERS_MOBILE', $arParams["SECTION_DISPLAY_PROPERTY"], $arParams["SECTION_BG"], "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN"));
if(LANGUAGE_ID == 'ua' && $section['UF_NAME_UA'])
...
if ($section) {
    ...
    if(LANGUAGE_ID == 'ua' && strlen($section['UF_DETAIL_TEXT_UA']))
    {
        $arSection['UF_SECTION_DESCR'] = $section['UF_DETAIL_TEXT_UA'];
        $arSection['DESCRIPTION'] = $section['UF_DETAIL_TEXT_UA'];
    }
	elseif(strlen($section["UF_SECTION_DESCR"])){
