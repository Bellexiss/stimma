...
<?if($arResult):?>
	<?if (!function_exists('showSubItemss')) {
		function showSubItemss($arParams = [
			...
			<?endif;?>
			<span class="name <?=$arParams['WIDE_MENU'] ? 'option-font-bold' : ''?>" style="<?=mb_strtolower($arParams['ITEM']["TEXT"]) == 'sale' ? 'color:#8B0000;' : ''?>"><?=$arParams['ITEM']["TEXT"]?></span><?=($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS'] ? CMax::showIconSvg("right light-ignore", SITE_TEMPLATE_PATH.'/images/svg/trianglearrow_right.svg', '', '') : '')?>
			<?if($arParams['ITEM']["LINK"]):?>
				</a>
			<?endif;?>
			...
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							<a class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" href="<?=$arItem['PARAMS']['URL'] ? $arItem['PARAMS']['URL'] : $arItem["LINK"]?>" <?=$arItem["LINK"] == '/catalog/' ? 'onclick="return false;"' : ''?>>
								<div>
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<div class="dropdown-menu wrb2 <?=$bRightSide ? 'with_right_block' : ''?> <?=$arItem['bManyItemsMenu'] ? 'long-menu-items' : ''?> <?=$arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']?>">
									...
									<div class="customScrollbar <?=($bWideMenu ? 'scrollblock' : '');?>">
										<?if($bRightSide && !$arItem['bManyItemsMenu'] && false):?>
												...
												<?if($MENU_TYPE == 2 && $bWideMenu):?>
													...
													<li class="<?=($arSubItem["SELECTED"] ? "active" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($bHasPicture ? "has_img" : "")?>">
														<?if($bHasPicture && $bWideMenu):
															...
															<?endif;?>
														<?endif;?>
														<a href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name option-font-bold nofb4" style="<?=mb_strtolower($arSubItem["TEXT"]) == 'sale' ? 'color:#8B0000;' : ''?>"><?=$arSubItem["TEXT"]?></span></a>
													</li>
												<?else: // type 1?>
													<?
