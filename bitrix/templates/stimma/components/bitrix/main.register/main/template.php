...
<div class="registraion-page pk-page">
	...
	<?else:?>
		...
		<?else:?>
        <?
            if(!isset($_SESSION['UF_PHONE_CODE']))
            {
                ?>
                <div class="form">
                    <form id="registraion-page-form" method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data" >
                        ...
                        $arResult["REQUIRED_FIELDS_FLAGS"]['EMAIL']='N';
                        ...
                        <div class="form_body">
                            <?foreach($arFields as $FIELD):?>
                                ...
                                <?if(($FIELD != "LOGIN" && $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") || $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] != "Y"):?>
                                    <div class="form-control"  style="<?=$FIELD == 'PASSWORD' || $FIELD == 'CONFIRM_PASSWORD' ? 'display:none;' : ''?>">
                                    <?
                                ...
                                <?switch ($FIELD){
                                    ...
                                    <?case "PERSONAL_PHONE":?>
                                        ...
                                        <?break;?>
                                    <?case "UF_UGROUP":?>
                                        <select id="input_<?=$FIELD;?>" name="REGISTER[<?=$FIELD?>]">
                                            <option value="25"><?=LANGUAGE_ID == 'ua' ? 'Роздрібний клієнт' : 'Розничный клиент'?></option>
                                            <option value="26"><?=LANGUAGE_ID == 'ua' ? 'Оптовий клієнт' : 'Оптовый клиент'?></option>
                                            <?/*<option value="27"><?=LANGUAGE_ID == 'ua' ? 'Дропшипер' : 'Дропшиппер'?></option>*/?>
                                        </select>
                                        <?break;?>
                                        <?break;
                                    default:?>
                            ...
                            <?if($arUFields):?>
                                <?foreach($arUFields as $arUField):?>
                                    <?
                                    if($arUField['FIELD_NAME'] == 'UF_REG_POPUP' && isset($_GET['choose']))
                                    {
                                        ?>
                                        <input size="20" class="fields string " name="UF_REG_POPUP" tabindex="0" type="hidden" value="Так">
                                        <input type="hidden" name="choose_from_popup" value="<?=$_GET['choose']?>">
                                        <?
                                    }
                                    elseif(!isset($_GET['choose']))
                                    {
                                        ?>
                                        <div class="form-control" style="<?=$arUField['FIELD_NAME'] == 'UF_REG_POPUP' ? 'display:none;' : ''?>">
            ...
            else
            {
                ...
                <form class="form" method="post">
                    <?
                    if(isset($_SESSION['MY_ERRORS']))
                    {
                        ?><div style="color:red;"><?=$_SESSION['MY_ERRORS']?></div><?
                    }
                    ?>
                    <div class="form-control error">
                        <label for="input_NAME"><span>Введіть код підтвердження<span class="star">*</span></span></label>
                        <label id="input_NAME-error" class="error" for="input_NAME">Заповніть це поле</label><input size="30" type="text" id="input_NAME" name="confirm_sms_code" required="" value="" aria-required="true" aria-invalid="true">
                        <button style="margin-top:20px;" class="btn btn-default btn-lg" type="submit" name="register_submit_button1" value="Y"><?=LANGUAGE_ID == 'ua' ? 'Підтвердити' : 'Подтвердить'?></button>
                    </div>
                </form>
                <?
            }
