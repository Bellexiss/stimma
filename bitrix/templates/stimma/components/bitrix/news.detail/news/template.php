...
<?
//if(LANGUAGE_ID == 'ua' && $arResult['PROPERTIES']['WIDE_TEXT_UA']['~VALUE']['TEXT'])
//    $arResult['FIELDS']['DETAIL_TEXT'] = $arResult['PROPERTIES']['WIDE_TEXT_UA']['~VALUE']['TEXT'];
...
<div class="detail_wrapper detail-news1">
	...
	<?if($bBannerChar): // banner char?>
		<?
		$arBannerChar = array(
            'TEXT' => LANGUAGE_ID == 'ua' ? $arResult['PROPERTIES']['BANNER_CHAR_TEXT_UA']['VALUE'] : $arResult['PROPERTIES']['BANNER_CHAR_TEXT']['VALUE'],
						...
						<div class="row flexbox">
							<div class="left_project_block col-md-<?=($bShowFormQuestion || $bShowFormOrder ? '9' : '12')?> col-sm-<?=($bShowFormQuestion || $bShowFormOrder ? '8' : '12')?> col-xs-<?=($bShowFormQuestion || $bShowFormOrder ? '7' : '12')?>">
								<?if(isset($arResult['DISPLAY_PROPERTIES']['DATA'])):?>
									<div class="date font_upper"><?=date('d.m.Y',strtotime($arResult['ACTIVE_FROM']))?><?//=$arResult['DISPLAY_PROPERTIES_FORMATTED']['DATA']['VALUE']?></div>
								<?endif;?>
						...
						<div class="info wti" >
							<?if($arResult['DISPLAY_PROPERTIES_FORMATTED'] || ($bShowFormQuestion || $bShowFormOrder)):?>
								<div class="row">
									<div class="col-md-12">
										<?if(isset($arResult['DISPLAY_PROPERTIES']['DATA'])):?>
											<div class="date font_upper muted"><?=date('d.m.Y',strtotime($arResult['ACTIVE_FROM']))?><?//=$arResult['DISPLAY_PROPERTIES_FORMATTED']['DATA']['VALUE']?></div>
										<?endif;?>
										<?if(isset($arResult['PROPERTIES']['TASK_PROJECT']) && $arResult['PROPERTIES']['TASK_PROJECT']['~VALUE']['TEXT']):?>
	...
	<div class="period_wrapper_inner">
		...
		<?if($bActiveDate):?>
			<div class="period-block darken ncolor font_xs <?=($arResult['ACTIVE_TO'] ? 'red' : '');?>">
				...
				<?else:*/?>
					<span class="date"><?=LANGUAGE_ID == 'ua' ? 'Опубліковано' : 'Опубликовано'?>: <?=date('d.m.Y',strtotime($arResult['ACTIVE_FROM']))?><?//=$arResult['DISPLAY_ACTIVE_FROM']?></span>
				<?//endif;?>
			</div>
		<?endif;?>
	</div>
...
<?if($bShowDetailContent):?>
	...
	?>
<h1 style="text-align: center;"><?=UA ? $arResult['PROPERTIES']['NAME_UA']['VALUE'] : $arResult['NAME']?></h1>
