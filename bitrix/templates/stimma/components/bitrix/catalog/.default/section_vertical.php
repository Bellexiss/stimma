...
if(!$bIndex)
{
    ...
    if($cache -> isStartCache($apppageCode, 86400*5))
    {
        ...
        $min = CIBlockElement::GetList(array('PROPERTY_MINIMUM_PRICE' => 'asc'), $filter+['>PROPERTY_MINIMUM_PRICE' => 0], false,['nTopCount' => 1], ['ID','IBLOCK_ID', 'CODE','PROPERTY_MINIMUM_PRICE','PREVIEW_PICTURE']) -> Fetch()['PROPERTY_MINIMUM_PRICE_VALUE'];
...
if(isset($_GET['faq']))
{
    ...
    # FAQ
    $questions=[];
    $res=$DB->Query('select * from faq_sections where UF_ID = ' . $thisSection['ID'] . ' order by ID asc');
    while ($record =$res->Fetch())
        $questions[] = $record;
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading" role="tab" id="heading1">
                                <h4 class="panel-title">
                                    <a class="collapsed" role="button" data-toggle="collapse" href="#collapse1" aria-expanded="false" >
                                    <span class="text">
    ...
    {
        $block3 = [];
        $res = CIBlockElement::GetList(array('PROPERTY_MINIMUM_PRICE' => 'asc'), $filter+['>PROPERTY_MINIMUM_PRICE' => 0], ['NAME'],['nTopCount' => 5]);
        while ($record = $res->GetNext())
            $block3[] = $record;

        $block2 = [];
        $res = CIBlockElement::GetList(array('PROPERTY_COUNTER' => 'desc','SHOW_COUNTER'=>'desc'), $filter+['>PROPERTY_MINIMUM_PRICE' => 0], ['NAME'],['nTopCount' => 5]);
        while ($record = $res->GetNext())
            $block2[] = $record;
        ?>
        <div class="cat-faq-block">
            ...
            <div class="cat-faq-list">
                <div class="panel-group" role="tablist" aria-multiselectable="false">
                    <div class="panel panel-default">
                        ...
                        <div id="collapse1" class="panel-collapse collapse" role="tabpanel" >
                            <div class="panel-body">
                                В наявності <?=$cnt?> товарних пропозицій за ціною від <?=intval($min)?> грн до <?=intval($max)?> грн
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div id="collapse2" class="panel-collapse collapse" role="tabpanel" >
                                ...
                                {
                                    ?><div class="panel-body"><a href="<?= $item['DETAIL_PAGE_URL'] ?>"><?=$item['NAME']?></a></div><?
                                    $jPart1.='<a href="'.$item['DETAIL_PAGE_URL'].'">'.$item['NAME'].'</a>';
                                }
                                ?>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading" role="tab" id="heading2">
                            <h4 class="panel-title">
                                <a class="collapsed" role="button" data-toggle="collapse" href="#collapse3" aria-expanded="false" >
                                <span class="text">
                        ...
                        <div id="collapse3" class="panel-collapse collapse" role="tabpanel" >
                                ...
                                {
                                    ?><div class="panel-body"><a href="<?= $item['DETAIL_PAGE_URL'] ?>"><?=$item['NAME']?></a></div><?
                                    $jPart2.='<a href="'.$item['DETAIL_PAGE_URL'].'">'.$item['NAME'].'</a>';
                                }
                                ?>
