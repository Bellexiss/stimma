...
?>
		...
		if ($elementId > 0 && !$bIndex)
		{
            ...
            $block = CIBlockElement::GetList(['rand'=>'asc'],['IBLOCK_ID'=>21,'SECTION_ID'=>$sid,'INCLUDE_SUBSECTIONS'=>'Y','PROPERTY_COLOR' => $thisEl['PROPERTY_COLOR_VALUE'],'>SORT'=>0,'ACTIVE'=>'Y'],false,['nTopCount'=>4],['ID','IBLOCK_ID','PROPERTY_COLOR']);
            $cnt = 0;
            $ids = [];
            ...
            if(count($ids) < 4)
            {
                $block = CIBlockElement::GetList(['rand'=>'asc'],['IBLOCK_ID'=>21,'SECTION_ID'=>$sid,'INCLUDE_SUBSECTIONS'=>'Y','>SORT'=>0,'ACTIVE'=>'Y'],false,['nTopCount'=>4],['ID','IBLOCK_ID','PROPERTY_COLOR']);
                while ($record = $block->Fetch())
                {
                    $ids[$record['ID']] = $record['ID'];
            ...
            if(!empty($ids))
            {
                ...
                <div class="main-googs-wraper mgw4">
                    <div class="main-googs-wraper-title"><?=LANGUAGE_ID == 'ua' ? 'Схоже' : 'Похоже'?></div>
                    <?
