...
function showItem($arItem, $arParams,$arResult)
{
...

    {
        ...
        //if($arItem['PROPERTIES']['CML2_LINK']['VALUE'])
        $slider = [$img];
        ...
        if(!empty($slider))
        {
            foreach($slider as $index => $itemItem)
            {
                $slider[$index] = convertToWebP($itemItem);
            }
        }
        ?>
        <div data-entity="scu" class="<?=$arParams['BLOCK_CLASS'] ? $arParams['BLOCK_CLASS'] : 'catalog-item-cont'?> <?=$dopClass?>" data-item="<?=$arItem['ID']?>" offer-item="<?=$arItem['OFFERS'][0]['ID']?>">
            <div class="catalog-item-block <?=($item['PRICES']['BASE']['DISCOUNT_VALUE'] > $minPrice['DISCOUNT_VALUE']) ? 'catalog-item-block-discount' : ''?>"  data-entity="scu-values">
                <div  class="catalog-item-img">
                    <div class="catalog-item-img-slider no-slick ciisns">
                        <?
                        foreach($slider as $src)
                        {
                            ?>
                    ...
                    <div class="catalog-item-size-list" data-code="RAZMER">
                        ...
                        foreach ($arItem['OFFERS'] as $indexOFfer => $offer)
                        {
                            ...
                            $sizes[$arResult['SORTING_SIZES'][$offer['PROPERTIES']['RAZMER']['VALUE_ENUM']]] = mb_strtoupper($offer['PROPERTIES']['RAZMER']['VALUE']);
                        }
                        ksort($sizes);
                    ...
                    <div class="card-badge-block">
                        ...
                        if(!$arParams['IS_NEW'] && !$arParams['IS_SALE'])
                        {
                            if(isset($_GET['pp']))
                            {
                                ?><pre>1 <?=print_r($USER -> GetGroups(), 1)?></pre><?
                            }
                            $gGroups = explode(',',$USER -> GetGroups());
                            if(isset($_GET['pp']))
                            {
                                ?><pre>2 <?=print_r($gGroups, 1)?></pre><?
                            }
                            if(!$gGroups) $gGroups = [2];
                            if(isset($_GET['pp']))
                            {
                                ?><pre>3 <?=print_r($gGroups, 1)?></pre><?
                                ?><pre>3 <?=print_r($item, 1)?></pre><?
                            }
                ...
                <div class="catalog-item-info">
                    <div class="catalog-item-price">
                        ...
                        {
                            ?><div class="catalog-item-price-old cip5"><?=$item['PRICES']['BASE']['PRINT_VALUE']?></div><?
                        }
                        ?>
                    </div>
                    <div class="catalog-item-colors cic5" data-code="COLOR_REF">
                        <?
