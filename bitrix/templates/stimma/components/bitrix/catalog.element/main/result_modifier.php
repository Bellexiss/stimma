...
foreach ($arResult['PROPERTIES'] as $index => $arItem)
{
	if(isset($arItem['VALUE_XML_ID'])|| $arItem['CODE'] == 'COLOR_REF')
	{
		$result = changeValue($arItem['IBLOCK_ID'], $arItem["CODE"], $arItem['VALUE_XML_ID'], $values_ua);
		$arResult['PROPERTIES'][$index]['VALUE'] = $result['values'];
		$arResult['PROPERTIES'][$index]['NAME'] = $result['name'];
	}
}
foreach ($arResult['OFFERS'] as $indexOffer => $offer)
{
	foreach ($offer['PROPERTIES'] as $index => $arItem)
	{
		if(isset($arItem['VALUE_XML_ID'])|| $arItem['CODE'] == 'COLOR_REF')
		{
			$result = changeValue($arItem['IBLOCK_ID'], $arItem["CODE"], $arItem['VALUE_XML_ID'], $values_ua);
			$arResult['OFFERS'][$indexOffer]['PROPERTIES'][$index]['VALUE'] = $result['values'];
			$arResult['OFFERS'][$indexOffer]['PROPERTIES'][$index]['NAME'] = $result['name'];
		}
	}
}
// $values_ua
			...
			if (!empty($arOffer['DISPLAY_PROPERTIES']))
			{
				...
				foreach ($arOffer['DISPLAY_PROPERTIES'] as &$arOneProp)
				{
					...
					$arSKUProps[] = array(
						...
						'HINT' => $arOneProp['HINT'],
						'VALUE_XML_ID' => $arOneProp['VALUE_XML_ID'],
					);
...
foreach ($arResult['JS_OFFERS'] as $indexOffer => $offer)
{
	foreach ($offer['DISPLAY_PROPERTIES'] as $index => $arItem)
	{
		if(isset($arItem['VALUE_XML_ID'])|| $arItem['CODE'] == 'COLOR_REF')
		{
			$result = changeValue(25, $arItem["CODE"], $arItem['VALUE_XML_ID'], $values_ua);
			$arResult['JS_OFFERS'][$indexOffer]['DISPLAY_PROPERTIES'][$index]['VALUE'] = $result['values'];
			$arResult['JS_OFFERS'][$indexOffer]['DISPLAY_PROPERTIES'][$index]['NAME'] = $result['name'];
		}
	}
}
