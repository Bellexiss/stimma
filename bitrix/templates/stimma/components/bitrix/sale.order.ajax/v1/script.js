function initChooseCity()
{
	...
	if($('#delivery_method').prop('checked') || $('#delivery_method3').prop('checked'))
		whereUrl = 'get_city_np.php';
...
$(document).ready(function()
{
	...
	$(document).on('change','[name=delivery_method]',function()
	{
		...
		if ( id_shipping != 14 && id_shipping != 17 ) {
			$('.svNovaPoshta').hide();
			console.log('hide');
		} else {
			$('.svNovaPoshta').show();
    ...
    $('select[name="choose_city_np"]').on('change', function() {
        if ($(this).closest('.form-control').hasClass('error-select') && $(this).val() !== '') {
            $(this).closest('.form-control').removeClass('error-select');
        }
    });


    $('select[name="choose_city_np_vid"]').on('change', function() {
        if ($(this).closest('.form-control.choose_city').hasClass('error-select') && $(this).val() !== '-1') {
            $(this).closest('.form-control.choose_city').removeClass('error-select');
        }
    });
...
document.querySelectorAll('.form-control input').forEach(function(input) {
	...
	$(document).on('click','.create_order',function()
	{
		...
		$npDelivery = $(form).find('#delivery_method').prop('checked');
		$ukrDelivery = $(form).find('#delivery_method1').prop('checked');
		$pickupDelivery = $(form).find('#delivery_method2').prop('checked');
        $npDeliveryP = $(form).find('#delivery_method3').prop('checked');

		city = $(form).find('[name=choose_city_np]').val();
		vidd = $(form).find('[name=choose_city_np_vid]').val();
		...
		if(second_name == '')
		{
			$(form).find('[name=second_name]').addClass('error');
			error = true;
		}
		if(fio == '')
		{
		...
		if(($npDelivery || $ukrDelivery || $npDeliveryP) && (!city || !vidd || city == -1 || vidd == -1))
		{
			if(!city || city == -1)$(form).find('[name=choose_city_np]').closest('.form-control').addClass('error-select');
			if(!vidd || vidd == -1)$(form).find('[name=choose_city_np_vid]').closest('.form-control').addClass('error-select');
			error = true;
		}
