	...
	BX.Sale.OrderAjaxComponent = {
		...
		sendRequest: function(action, actionData)
		{
			...
			if (action === 'saveOrderAjax')
			{
				...
				{
					error = true;
					$('.lic_condition').before('<div class="order_error" style="color:red;">У Вас в коризні товари, яких немає в наявності. </div>')
					this.endLoader();
				}
				else $('.order_error').remove();
			...
			{
				console.log('window.use_coupon');
				console.log(window.use_coupon);
				if(action == 'enterCoupon' && (window.use_coupon == 0 || window.use_coupon == '0'))
				{
					if(location.pathname.indexOf('/ru/') > -1)
						$('.bx-soa-coupon').append('<div class="error_coupon" style="color: #8B0000;"></div>')
					else
						$('.bx-soa-coupon').append('<div class="error_coupon" style="color: #8B0000;"></div>')
					this.endLoader();
				}
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			var headers = [
					BX.create('TD', {
						props: {className: 'bx-soa-item-td bsii33'},
						style: {paddingBottom: '5px'},
						children: [
							BX.create('DIV', {
								props: {className: 'bx-soa-item-td-title bsi1'},
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				headers.push(
					BX.create('TD', {
						...
						children: [
							BX.create('DIV', {
								props: {className: 'bx-soa-item-td-title bsi2'},
								text: column.name
							})
						]
					})
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			cols = [
				BX.create('TD', {
					props: {className: 'bx-soa-item-td bsii11'},
			...
			if (hiddenColumns.length)
			{
				basketItemsNode.appendChild(
					BX.create('TR', {
						...
						children: [
							BX.create('TD', {
								props: {className: 'bx-soa-item-td bssii22'},attrs:{'colspan': countCols},
								children: [
									BX.create('A', {
			...
			if (this.options.showPropsInBasket && props.length)
			{
				for (var i in props)
				{
					if (props.hasOwnProperty(i))
					{
						...
						propsNodes.push(
							BX.create('DIV', {
								props: {className: 'bx-soa-item-td-title bsi3'},
		...
		editCoupons: function(basketItemsNode)
		{
			...
			/*var couponHeader = BX.create('DIV',{
				props: {className: 'bx-soa-section-title-container'},
				children: [
					BX.create('H2',{
						...
						children: [
							BX.create('span',{
								props: {className: 'bx-soa-section-title-count couponcount'}
							})
						]
					})
				]
			...
			//$('.couponcount').text('Промокод');

			$('.blockCOuponNode').remove();
			$('#bx-soa-coupon .bx-soa-section-content').before('<div class="blockCOuponNode bx-soa-section-title-container">\n' +
				'        <h2 class="bx-soa-section-title col-sm-9">\n' +
				'         <span class="bx-soa-section-title-count"></span>Промокод        </h2>\n' +
				'       </div>');
		},
