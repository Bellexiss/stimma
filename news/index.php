...
if ($_SERVER['REQUEST_URI'] == '/news/' || $_SERVER['REQUEST_URI'] == '/news') {
    LocalRedirect('/blog/', true, '301 Moved Permanently');
    exit;
}



$APPLICATION->SetTitle("Про нас пишуть");
?>
...
else
{
    ...
    if($matches[1])
    {
        ...
        if($news = $news->GetNextElement())
        {
            ...
            $APPLICATION->SetPageProperty('title',$props['NAME_UA']['VALUE'].' - Новини Stimma');
            //$APPLICATION->SetPageProperty('description',strip_tags($props['BNR_DOP_TEXT_UA']['~VALUE']['TEXT']));
            $APPLICATION->SetPageProperty('description',strip_tags($props['NAME_UA']['VALUE']).' - Новини Інтернет магазину одягу від виробника Stimma');

            if(empty(trim($props['WIDE_TEXT_UA']['~VALUE']['TEXT'])))
            {
